...
// `crypto.randomUUID` is unavailable. Bound the length so a caller
// cannot stuff arbitrarily large ids into log lines.
...
// Stamps every request with a stable id, binds a child logger carrying
// that id, and emits a structured access log after the handler resolves.
