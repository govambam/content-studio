...
--
-- Postgres rejects `alter type ... add value` inside an explicit
-- transaction block, so this file intentionally runs as top-level
-- statements rather than wrapping everything in BEGIN/COMMIT.
...
-- `webhook_url` is a secret, so no policies are granted to the
-- `authenticated` role. All reads and writes flow through the API,
-- which uses the service role to manage this row. The API exposes
-- a redacted summary to clients; authenticated JWTs have no direct
-- access at all.
...
-- Extend the activity_event_type enum. This must be a top-level
-- statement (not inside a transaction block) so the API's insert path
-- can see the new value on the next migration run.
