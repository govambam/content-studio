...
-- `webhook_url` is a secret, so authenticated clients must not be
-- able to SELECT it directly — the API exposes a redacted summary
-- instead. Authenticated users can still insert/update/delete the
-- row via the API's service role, but the anon/authenticated JWTs
-- that clients use are restricted to write-only operations here.
create policy "Authenticated users can insert slack_integrations"
  on slack_integrations for insert to authenticated with check (true);
create policy "Authenticated users can update slack_integrations"
  on slack_integrations for update to authenticated using (true) with check (true);
create policy "Authenticated users can delete slack_integrations"
  on slack_integrations for delete to authenticated using (true);
